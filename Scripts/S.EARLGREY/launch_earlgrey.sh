...
#SBATCH --mem=440Gb           # Memory pool for all cores (see also --mem-per-cpu)
...
## round 1
earlGrey -g ${dir_fasta} -s ${species_name} -o ${dir_out} -t 16 -c yes -m yes -n 20 -a 5 -e yes -q yes -f 1000

# ## round 2 if using a previously identified library
# earlGrey -g ${dir_fasta} -s ${species_name} -o ${dir_out} -t 64 -c yes -m yes -n 20 -a 5 -e yes -q yes -f 1000 -l ${premasking_library_fa}