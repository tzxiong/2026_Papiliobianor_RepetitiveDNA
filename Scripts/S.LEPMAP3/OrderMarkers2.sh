...
#SBATCH -o OMarker_%j.out  # File to which STDOUT will be written, %j i$
#SBATCH -e OMarker_%j.err  # File to which STDERR will be written, %j i$
...
zcat ${dir_call_data}/data.call.with_pseudoGrandparents.fixedDiff_only.chr.${chromosome}.gz | java -Xmx90G -cp ~/Software/LepMap3/bin/ OrderMarkers2 evaluateOrder=${dir_order_file}/order.with_pseudoGrandparents.fixedDiff_only.chr.${chromosome}.txt data=- recombination2=0 recombination1=${recombination} interference1=${interference} outputPhasedData=1 grandparentPhase=${grandparentPhase} useMorgan=1 improveOrder=0 numThreads=10 families=${f} >${dir_output}/impute.with_pseudoGrandparents.fixedDiff_only.grandparentPhase.${grandparentPhase}.recombination1.${recombination}.interference1.${interference}.${families}.chr.${chromosome}.txt